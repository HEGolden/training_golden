...
## Interactive table
```{r}
datatable(locations) # provides a searchable interactive table
```

## Interactive Maps with 'leaflet'
similar to ggplot but works with pipe not plus
```{r}
leaflet(locations) %>% 
  addTiles() %>% 
...
Changing the basemape from default (open street map) to the WMS tiles, with various attributes
```{r}
leaflet(locations) %>% 
